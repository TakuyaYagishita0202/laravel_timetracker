<template>
    <div class="col-md-8 col-md-offset-2">
                    ...
                    <ul class="list-group">
                        <li class="list-group-item clearfix">
                            ...
                            <div class="pull-right">
                                ...
                                <!-- <button
                                    v-if="showTimerForProject(project, timer)"
                                    class="btn btn-sm btn-danger"
                                    @click="stopTimer()"
                                >
            ...
            <div v-if="projects.length > 0">
                ...
                >
                    ...
                    <div class="panel-body">
                        ...
                        <p v-else>
                            Nothing has been recorded for "{{ project.name }}".
                ...
                <div v-if="timers.length > 0">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <ul class="list-group" v-if="timers.length > 0">
                                ...
                                >
                                    ...
                                    <span
                                        :style="{
                                            backgroundColor:
                                                timer.category_color
                                        }"
                                        style="display: inline-block; height:10px; width:10px; "
                                    ></span>
                                    <span
                                        :style="{ color: timer.category_color }"
                                        >{{ timer.category_name }}</span
                                    >
                                    <div>
                            ...
                            <p v-else>
                                まだ記録がありません。タイマーをセットして記録を始めましょう。
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <button
                                ...
                                v-bind:disabled="
                                    newTimerName === '' ||
                                        newTimerCategory === ''
                                "
                                @click="createTimer(selectedProject)"
                                type="submit"
        ...
        <div class="timers" v-else>Loading...</div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        stopTimer: function() {
            window.axios
                .post(`/projects/${this.counter.timer.id}/timers/stop`)
                .then(response => {
                    // Loop through the projects and get the right project...
                    this.projects.forEach(project => {
                        if (
                            project.id ===
                            parseInt(this.counter.timer.project.id)
                        ) {
                            // Loop through the timers of the project and set the `stopped_at` time
                            return project.timers.forEach(timer => {
                                if (
                                    timer.id === parseInt(this.counter.timer.id)
                                ) {
                                    return (timer.stopped_at =
                                        response.data.stopped_at);
                                }
                            });
                        }
                    });
        ...
        createTimer: function(project) {
            window.axios
                .post(`/projects/${project.id}/timers`, {
        ...
        createProject: function() {
            window.axios
                .post("/projects", { name: this.newProjectName })
